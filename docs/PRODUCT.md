...
| Report branding | **Three-level fallback (confirmed 2026-09-01): store branding → org branding → Raporo default.** Nothing is ever unbranded; a single-identity org configures once and every store inherits; a multi-identity org (e.g. one owner running a food shop and a clothing shop) gives each store its own logo/colors. Refinements: (a) explicit per-store `use_own_branding` toggle so an empty field means *inherit*, never "deliberately blank" — with the toggle on, per-field fallback still applies, so an owner can override just the logo and keep the org's colors; (b) **store name is always local** (every store has its own name and that heads its report) — only logo, colors and typography travel down the chain; (c) the consolidated cross-store report carries **org** branding by definition, being an org-level document. |
| Auth | Login with username OR email OR phone, + password. **Email is required for every account** (2026-09-01) — it is the password-reset channel for all users: emailed link, single-use, 1h expiry, non-enumerating. Phone stored with country code, digits only, no `+`. **Maximum-security posture (2026-09-01): argon2id hashing, login rate-limiting + lockout/backoff, secure session cookies, safe password reset, no user enumeration — fast for users, hostile to attackers.** Social login later — architecture leaves space, nothing built now. |
